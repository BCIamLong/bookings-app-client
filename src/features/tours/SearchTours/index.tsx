  ...
  return (
    <div className="relative flex items-center flex-col">
      <div className='relative w-[40rem] mt-3  flex items-center justify-center'>
        <input className="px-3 py-2 bg-stone-0 mb-6 rounded-full w-full outline-none border-brand-300 focus:border-brand-600 border-[1.5px] text-stone-600 h-10" placeholder="Search posts..." type="text" value={searchStr} onChange={(e) => setSearchStr(e.target.value)} />
        <div className=''>
        ...
        {tours?.length &&
          <ul className='absolute top-12 bg-stone-100 w-[99%] text-stone-600 rounded-lg py-2 px-3 flex flex-col gap-1'>
            {tours?.map((tour: ITour) =>
              <li><ButtonLink type='' href={`/tours/${tour._id}`}>{tour.name}</ButtonLink></li>)
            }
          </ul>
        }
      </div>
